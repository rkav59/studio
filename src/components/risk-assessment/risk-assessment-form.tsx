...
const riskControlItemSchema = z.object({
  ...
  value: z.string().max(1000, "Item too long, 1000 characters maximum.").optional(), 
});
...
export function RiskAssessmentForm({ onSaveAssessment, initialData, onCancel, className }: RiskAssessmentFormProps) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className={cn("space-y-8 py-4", className)}> {/* Reverted: Removed flex, added space-y and py-4 */}
        {/* Reverted: No ScrollArea directly here */}
          <FormField
            control={form.control}
            name="activity"
            render={({ field }) => (
        ...
        <div className="flex flex-wrap gap-2 pt-4 border-t"> {/* Reverted: Buttons are direct children of form */}
