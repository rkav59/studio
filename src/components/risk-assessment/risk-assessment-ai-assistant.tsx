...
const riskAssessmentFormSchema = z.object({
  ...
  potentialHazards: z.string().min(5, {
    message: "Potential hazards must be at least 5 characters.",
  }).max(2000, { message: "Potential hazards must be less than 2000 characters."}),
});

type RiskAssessmentFormValues = z.infer<typeof riskAssessmentFormSchema>;

export function RiskAssessmentAiAssistant() {
  ...
  async function onSubmit(data: RiskAssessmentFormValues) {
    ...
    try {
      const input: RiskAssessmentSuggestionInput = {
        ...
        identifiedHazards: data.potentialHazards,
      };
        ...
        <CardContent>
            <Form {...form}>
                ...
                <FormField
                    ...
                    render={({ field }) => (
                    <FormItem>
                        <FormLabel>Known Potential Hazards</FormLabel>
                        <FormControl>
                        <Textarea
      ...
      {suggestion && (
        <Card className="mt-8 shadow-lg">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
                <h3 className="font-semibold text-lg mb-1">Potential Risks Identified:</h3>
                <pre className="whitespace-pre-wrap rounded-md bg-secondary p-4 text-sm font-mono leading-relaxed">
                {suggestion.potentialRisks}
                </pre>
            </div>
            <div>
                <h3 className="font-semibold text-lg mb-1">Recommended Control Measures:</h3>
                <pre className="whitespace-pre-wrap rounded-md bg-secondary p-4 text-sm font-mono leading-relaxed">
