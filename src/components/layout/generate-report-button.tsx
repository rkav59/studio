...
function markdownToHtml(markdown: string): string {
  ...
  return html.split(/\n\s*\n/).map(paragraph => {
    ...
    if (trimmedParagraph.startsWith('<h') || trimmedParagraph.startsWith('<ul') || trimmedParagraph.startsWith('<ol') || trimmedParagraph.startsWith('<li') || trimmedParagraph.startsWith('<blockquote') || trimmedParagraph.startsWith('<pre') || trimmedParagraph.startsWith('<hr') || trimmedParagraph.startsWith('<table')) {
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col">
          ...
          <ScrollArea className="flex-grow my-4 pr-2">
            <div className="printable-area prose dark:prose-invert prose-sm sm:prose-base max-w-none leading-relaxed"
