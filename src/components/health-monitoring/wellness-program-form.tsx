...
import { CalendarIcon, Save, XCircle, Award, Users, PlayCircle, Info, Users2, Percent } from "lucide-react";
          ...
          <ScrollArea className="flex-1">
            <div className="p-6 space-y-6">
              <FormField control={form.control} name="programName" render={({ field }) => (
                  <FormItem><FormLabel className="flex items-center gap-1"><Award className="h-4 w-4"/>Program Name</FormLabel><FormControl><Input placeholder="e.g., Healthy Eating Challenge, Mental Wellness Workshops" {...field} /></FormControl><FormMessage /></FormItem>
              )}/>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <FormField control={form.control} name="startDate" render={({ field }) => (
                      <FormItem className="flex flex-col"><FormLabel className="flex items-center gap-1"><CalendarIcon className="h-4 w-4"/>Start Date</FormLabel>
                  ...
                  <FormField control={form.control} name="endDate" render={({ field }) => (
                      <FormItem className="flex flex-col"><FormLabel className="flex items-center gap-1"><CalendarIcon className="h-4 w-4"/>End Date (Optional)</FormLabel>
                      <Popover><PopoverTrigger asChild><FormControl>
                          <Button variant="outline" className={cn("w-full pl-3 text-left font-normal", !field.value && "text-muted-foreground")}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="targetAudience" render={({ field }) => (
                    <FormItem><FormLabel className="flex items-center gap-1"><Users2 className="h-4 w-4"/>Target Audience (Optional)</FormLabel><FormControl><Input placeholder="e.g., All Employees, Night Shift Staff" {...field} /></FormControl><FormMessage /></FormItem>
                )}/>
              </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <FormField control={form.control} name="actualParticipants" render={({ field }) => (
                      <FormItem><FormLabel className="flex items-center gap-1"><Percent className="h-4 w-4"/>Actual Participants (Optional)</FormLabel><FormControl><Input type="number" placeholder="e.g., 75" {...field} /></FormControl><FormMessage /></FormItem>
                  )}/>
              </div>
              <FormField control={form.control} name="participationNotes" render={({ field }) => (
                  <FormItem><FormLabel className="flex items-center gap-1"><Info className="h-4 w-4"/>Participation Notes (Optional)</FormLabel><FormControl><Textarea placeholder="General notes about participation numbers, engagement, feedback, etc." rows={3} {...field} /></FormControl><FormMessage /></FormItem>
              )}/>
            </div>
          </ScrollArea>
